...
-- 🚀 TELEGUIADO DE VELOCIDADE 24
-- 🍋 LEMON HUB ADICIONADO
-- 🔍 SERVER FINDER 1M-10M
-- 📡 TELEPORT SCRIPT ADICIONADO
-- 🔑 AUTO MOREIRA ADICIONADO (PRIMEIRO)
...
-- ============================================
-- SISTEMA DE TELEGUIADO DE VELOCIDADE 24
-- ============================================

-- Variáveis do teleguiado
local targetPosition = nil
local positionMarker = nil
local isTeleporting = false
local teleportConnection = nil
local TELEPORT_SPEED = 24 -- VELOCIDADE 24 CONFORME PEDIDO

print("🎯 TELEGUIADO CONFIGURADO - VELOCIDADE: " .. TELEPORT_SPEED)
...
-- ============================================
-- LISTA DE SCRIPTS (ATUALIZADA COM AUTO MOREIRA EM PRIMEIRO)
-- ============================================

local mainScripts = {
    -- AUTO MOREIRA ADICIONADO EM PRIMEIRO LUGAR
    {name = "AUTO MOREIRA", icon = "🔑", color = Color3.fromRGB(255, 100, 100), 
     url = "https://keyexs.com/lejax7xmoreira/raw", badge = "KEY", special = "moreira",
     key = "LEJAX-MOREIRA-Y6I-32G"},
    
    -- Scripts originais mantidos (deslocados uma posição)
    ...
    -- SCRIPTS ADICIONADOS ANTERIORMENTE
    {name = "LEMON HUB", icon = "🍋", color = Color3.fromRGB(255, 255, 0), 
     url = "https://api.luarmor.net/files/v3/loaders/c4281c3937ebd537cb9e860182e41141.lua", badge = "HOT"},
    
    {name = "SERVER FINDER 1M-10M", icon = "🔍", color = Color3.fromRGB(0, 255, 150), 
     url = "https://raw.githubusercontent.com/rexzysab/stealabrainrot/refs/heads/main/main.lua", badge = "NEW"},
    
    {name = "TELEPORT SCRIPT", icon = "📡", color = Color3.fromRGB(150, 100, 255), 
     url = "https://tcscripts.discloud.app/scripts/servertp", badge = "TP"}
}
...
local helperFunctions = {
    ...
    {
        name = "TELEGUIADO VEL 24", 
        icon = "🚀", 
        color = Color3.fromRGB(100, 200, 255), 
        description = "Vai até posição marcada (vel 24)",
...
local function createButton(buttonData, index, container, isHelper)
    ...
    if buttonData.badge then
        ...
        elseif badgeText == "HOT" then
            badgeColor = Color3.fromRGB(255, 50, 50)
        elseif badgeText == "TP" then
            badgeColor = Color3.fromRGB(150, 100, 255)
        else
            badgeColor = Color3.fromRGB(100, 100, 100)
        end
        
        local badgeWidth = settings.badgeSize + (badgeText == "NEW" and 6 or (badgeText == "HOT" and 8 or (badgeText == "TP" and 10 or 0)))
...
for i, scriptData in ipairs(mainScripts) do
    ...
    button.MouseButton1Click:Connect(function()
        ...
        local success, err = pcall(function()
            ...
            if scriptData.special == true then
                getgenv().ServerDestroyerV6 = {Comprar = false, Spam = true}
            elseif scriptData.special == "moreira" then
                -- Configurar a KEY para Auto Moreira
                KEY = scriptData.key or "LEJAX-MOREIRA-Y6I-32G"
            end
        ...
        if success then
            ...
            -- Notificação especial para scripts importantes
            if scriptData.name == "AUTO MOREIRA" then
                task.wait(0.5)
                showNotification("🔑 AUTO MOREIRA ATIVADO! KEY: " .. (scriptData.key or "LEJAX-MOREIRA-Y6I-32G"), Color3.fromRGB(255, 100, 100))
            elseif scriptData.name == "LEMON HUB" then
                task.wait(0.5)
                showNotification("🍋 LEMON HUB CARREGADO!", Color3.fromRGB(255, 255, 0))
            elseif scriptData.name == "SERVER FINDER 1M-10M" then
                task.wait(0.5)
                showNotification("🔍 SERVER FINDER ATIVADO!", Color3.fromRGB(0, 255, 150))
            elseif scriptData.name == "TELEPORT SCRIPT" then
                task.wait(0.5)
                showNotification("📡 TELEPORT SCRIPT ATIVADO!", Color3.fromRGB(150, 100, 255))
            elseif scriptData.name == "Auto Block on Steal" then
                task.wait(0.5)
            ...
            showNotification("❌ Erro ao carregar " .. scriptData.name, Color3.fromRGB(255, 50, 50))
        end
...
print("✅ " .. #mainScripts .. " scripts criados (Auto Moreira em primeiro!)")
...
local function createPositionMarker(position)
    ...
    positionMarker.Size = Vector3.new(3, 3, 3)
    positionMarker.Position = position + Vector3.new(0, 1.5, 0)
    ...
    positionMarker.Transparency = 0.4
    ...
    billboard.Name = "MarkerText"
    billboard.Size = UDim2.new(0, 100, 0, 40)
    billboard.StudsOffset = Vector3.new(0, 3, 0)
    billboard.AlwaysOnTop = true
    billboard.Adornee = positionMarker
    
    local textLabel = Instance.new("TextLabel")
    textLabel.Size = UDim2.new(1, 0, 1, 0)
    textLabel.BackgroundTransparency = 1
    textLabel.Text = "📍 MARCADO"
    textLabel.TextColor3 = Color3.new(1, 1, 1)
    textLabel.TextSize = 14
    textLabel.Font = Enum.Font.GothamBold
    textLabel.Parent = billboard
    
    billboard.Parent = positionMarker
    
    -- Rotação suave
    task.spawn(function()
        while positionMarker and positionMarker.Parent do
            positionMarker.CFrame = positionMarker.CFrame * CFrame.Angles(0, math.rad(1.5), 0)
            task.wait()
        end
    end)
...
-- FUNÇÃO DE TELEPORTE COM VELOCIDADE 24
local function teleportWithSpeed()
    ...
    -- Cancelar teleporte anterior se houver
    if isTeleporting and teleportConnection then
        teleportConnection:Disconnect()
        isTeleporting = false
    end
    
    showNotification("🚀 TELEGUIADO INICIADO (Vel 24)", Color3.fromRGB(100, 200, 255))
    isTeleporting = true
    ...
    local travelTime = distance / TELEPORT_SPEED
    local startTime = tick()
    
    print("📍 Teleporte: Distância = " .. math.floor(distance) .. ", Tempo = " .. math.floor(travelTime) .. "s")
    
    -- Conectar ao Heartbeat para movimento suave
    teleportConnection = RunService.Heartbeat:Connect(function(deltaTime)
        if not isTeleporting or not character or not humanoidRootPart then
            teleportConnection:Disconnect()
            return
        end
        
        local elapsedTime = tick() - startTime
        local progress = math.min(elapsedTime / travelTime, 1)
        
        -- Calcular nova posição baseada no progresso
        local newPosition = startPos:Lerp(targetPosition, progress)
        ...
        if progress >= 1 then
            ...
            print("✅ Teleporte concluído em " .. math.floor(elapsedTime) .. " segundos")
        end
    end)
    ...
    button.MouseButton1Click:Connect(function()
        ...
        elseif helperData.functionType == "teleflight" then
            if isTeleporting then
                -- Cancelar teleguiado atual
                isTeleporting = false
                if teleportConnection then
                    teleportConnection:Disconnect()
                end
                showNotification("✋ TELEGUIADO CANCELADO", Color3.fromRGB(255, 150, 50))
            else
                -- Iniciar teleguiado com velocidade 24
                teleportWithSpeed()
            end
        end
    end)
...
CloseButton.MouseButton1Click:Connect(function()
    -- Cancelar teleguiado se estiver ativo
    if isTeleporting and teleportConnection then
        teleportConnection:Disconnect()
        isTeleporting = false
    end
...
footer.Text = "🎯 " .. #mainScripts .. " Scripts | 🚀 Teleguiado Vel " .. TELEPORT_SPEED .. " | " .. (isMobile and "📱" or "💻")
...
print("══════════════════════════════════════════════")
print("🍎 APPLE HUB - COM AUTO MOREIRA EM PRIMEIRO!")
print("📱 Plataforma: " .. (isMobile and "MOBILE" or "PC"))
print("🎯 Scripts: " .. #mainScripts .. " disponíveis")
print("✨ PRIMEIRO SCRIPT: 🔑 AUTO MOREIRA")
print("🔑 KEY: LEJAX-MOREIRA-Y6I-32G")
print("🚀 TELEGUIADO: VELOCIDADE " .. TELEPORT_SPEED .. " (CONFORME PEDIDO)")
print("📍 Marcador: Visual azul com texto flutuante")
print("⚡ Tecnologia: Movimento suave por Heartbeat")
...
print("══════════════════════════════════════════════")
print("🎮 SCRIPTS ADICIONADOS (EM ORDEM):")
print("1. 🔑 AUTO MOREIRA - Com KEY configurada")
print("2. 👑 Nameless")
print("3. 🌶️ Chilli")
print("4. ⚡ UCT")
print("5. 🏔️ Kurd")
print("6. 💀 Lag + Aura")
print("7. 👑 Admin Spam")
print("8. 🔓 Chilli Private")
print("9. ⚡ Speed Steal")
print("10. 🏓 Auto Ping Pong")
print("11. 🎯 Paintball")
print("12. 🎵 Lennon Hub")
print("13. 🔧 LKZ HELPER [OP]")
print("14. ⚡ Miranda Hub")
print("15. ⚡ Miranda 2")
print("16. 👟 ADIDAS PACK")
print("17. 🛡️ Auto Block on Steal")
print("18. 🍋 LEMON HUB")
print("19. 🔍 SERVER FINDER 1M-10M")
print("20. 📡 TELEPORT SCRIPT")
print("══════════════════════════════════════════════")
print("🎮 COMO USAR AUTO MOREIRA:")
print("1. Clique em 'AUTO MOREIRA' (primeiro botão)")
print("2. A KEY será automaticamente configurada")
print("3. O script será carregado com sucesso!")
print("4. KEY: LEJAX-MOREIRA-Y6I-32G")
print("══════════════════════════════════════════════")
